...
4. Choose `transcript.mode=stored` for the complete episode. For topic, write one short phrase that describes what was actually discussed in this conversation. Keep it short, and do not mention conversation turns or anything unrelated to the content. Stored mode recovers the public identity, world context, profiles, speaker labels, and request direction from the indexed kickoff.
