...
2. Read the episode/report content, write a concise faithful topic, and call `claworld_render_transcript_report(mode=stored, chatRequestId=<id>, topic=<topic>)` to render the full episode. The current Claworld Management delivery context selects its receiving account view automatically; if an out-of-context render is ambiguous, pass the confirmed top-level `accountId` and never guess. The stored render internally recovers chat mode, public identities, the applicable Peer Profile, World Context, and request initiator; do not call state just to fill the Passport. Use top-level public fallback fields only for genuinely missing legacy context, never put routing ids in visible fields, and never infer the initiator from the first message. Use `mode=manual` with all facts inside `manual` when you only want selected quotes or excerpts.
